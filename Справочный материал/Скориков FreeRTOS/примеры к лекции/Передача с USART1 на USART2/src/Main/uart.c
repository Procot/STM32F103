...
void USART_Config()
{
    // подаем тактирование на USART1, USART2, GPIOA, GPIOD
    ...
    // создаем очередь для пересылки сообщений с одного модуля 
    // USART на другой
    ...
    // объявляем и инициализируем структуру, описывающую параметры USART
    ...
    // инициализируем USART1, скорость обмена 9600 бод/с
    usart.USART_BaudRate = 9600;
    USART_Init(USART1, &usart);
    // инициализируем USART1, скорость обмена 115200 бод/с
    usart.USART_BaudRate = 115200;
    USART_Init(USART2, &usart);
    USART_Cmd(USART1, ENABLE); // разрешаем работу USART1
    USART_Cmd(USART2, ENABLE); // разрешаем работу USART2
    ...
    // конфигурируем линии портов ввода-вывода общего назначения    
...
void USART2_IRQHandler()
{
    ...
    {	
        ...
        // помещаем полученный байт данных во временную переменную
        uint8_t usartData = USART_ReceiveData(USART2);	
        // пересылаем данные, полученные с USART2, на USART1
        
        // 1 способ: без использования очереди        
        //USART_SendData(USART1, usartData);
        //while(USART_GetFlagStatus(USART1, USART_FLAG_TXE) == RESET);
        //
        
        // 2 способ: с использованием очереди
